...
Roo.prototype.duo = function(glob) {
  var bundle = require('duo-bundle/lib/bundle.js');
  var root = this.root || this.cwd;
  var glob = resolve(root, glob);
  var files = Glob(glob, { cwd: root });
  var self = this;

  // create routes for each file
  files.forEach(function(file) {
    var path = join('/', relative(root, file));
    self.get(path, function *(next) {
      yield bundle(root, file);
      return yield next;
    })
  });
